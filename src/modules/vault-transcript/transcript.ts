 ...
 *
 * Opt-in per agent group via `vault_transcript_enabled` (migration 030,
 * gated in `appendTranscriptTurn` below) — same shape as projected-sessions'
 * `projected_sessions_enabled`. `ncl vault-transcript enable|disable|list`
 * (index.ts) toggles it. Every new group still gets `transcript-append-host`
 * seeded (group-init.ts) so enabling later needs no restart-and-reseed step,
 * but an unenabled group's turns never reach execHostShim at all — no
 * subprocess, no "VAULT_PATH not configured" warning spam.
 *
...
async function appendTranscriptTurn(
  ...
  if (!isEnabled(agentGroupId)) return;
...
export async function appendDeliveredOutboundTurn(
  ...
  const speaker = recipientName ? `${assistantName} → ${recipientName}` : assistantName;
  await appendTranscriptTurn(agentGroupId, speaker, timestampIso, text);
}
