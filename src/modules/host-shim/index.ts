 ...
 * matching `<name>-host` script (./exec.ts, the actual whitelist check,
 * scoped to the calling session's own agent group) and writes the result
 * back as a `host_shim_response` system message with trigger=0 so it's an
 * inline reply to the tool call, not a wake.
 *
 * Guard always allows (./guard.ts) — the per-agent-group whitelist-file
 * check is the real security boundary, not a human approval gate.
 */
