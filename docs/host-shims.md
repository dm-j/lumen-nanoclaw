...
- **`--system-prompt-file` mode needs its own `--disallowedTools` write-access denylist.** Selecting an agent's persona via `--agent <name>` also enforces that agent's own `tools:` frontmatter restriction; folding the same persona into `--system-prompt-file` does not carry that restriction over — `claude` never reads the stripped frontmatter, so `--permission-mode bypassPermissions` alone would grant full read/write access regardless of what the source agent declares. Any shim using `--system-prompt-file` with a read-only-by-design persona must explicitly pass `--disallowedTools Write,Edit,NotebookEdit,Task,Agent` (or whatever subset applies) to actually enforce it. Caught 2026-09-16 as a live gap in both `briefing-host` (trunk template still missing this) and the first draft of `recall-host`'s conversion.
