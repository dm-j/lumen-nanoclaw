...
15. [Modular context injection: ordered segments](roadmap/context-injection-segments.md) — replace the one-string `wake_script` with a shared host-side runner over per-group ordered, independent segments (`notes`, `todo`, ...), with caps, failure markers and a preview command; Routine gets its own short list; design agreed 2026-09-23, full assembler specification written 2026-09-24 (segment contract, assembly rules, two-stage adoption starting instance-only via a `context-assemble` host-shim), nothing built
