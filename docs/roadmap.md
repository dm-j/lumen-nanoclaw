...
14. [Calendar event-note body: Teams signature + protected notes](roadmap/calendar-event-note-body.md) — screen the Teams signature down to join link/Meeting ID/Passcode/Phone Conference ID and put the upstream text in a `^event-desc` quote block sync owns, so nothing typed below it is ever overwritten; spec for the vault project's Claude (raised 2026-09-23), nothing built
