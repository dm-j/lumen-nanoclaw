...
1. [Session-sync WebSocket transport](session-sync-transport.md) — replaces bind-mounted session DBs (recurring macOS SQLite corruption / `DB_RETRY_EXHAUSTED`); three canary rounds on 2026-08-16 found and fixed 7 real bugs total, plus the `delivered.sync_acked` reconnect-backfill gap (§8.2 item 4); all hard prerequisites are done and Lumen is running live on `'sync'` transport with confirmed working round trips — next step (§8.7) is the observation window elapsing clean, then widening to a second non-critical group
