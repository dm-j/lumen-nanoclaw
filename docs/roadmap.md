...
1. [Session-sync WebSocket transport](session-sync-transport.md) — replaces bind-mounted session DBs (recurring macOS SQLite corruption / `DB_RETRY_EXHAUSTED`); first real canary attempt 2026-08-16 found and fixed 4 real bugs (TLS cert SAN, Bun's `ws` silently dropping `ca`/`rejectUnauthorized`, a silent-hang in `applyInboundRow`, `releaseStagedMessage` not pushing) but rolled back — destinations/session_routing have no on-connect backfill, so a container's local destination map is permanently empty; **not safe for any group with real traffic until that's built (§8.6)**
