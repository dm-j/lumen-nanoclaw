...
9. [Host-shim templates missing 2026-09-16 fixes](roadmap/host-shim-templates-drift.md) — trunk `recall-host`/`briefing-host` templates predate the `claude` CLI output-format fix, the `--disallowedTools` write-access hardening, and PrefixRouter routing; a new install today seeds broken/insecure copies
