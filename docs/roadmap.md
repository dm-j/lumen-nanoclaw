...
1. [Session-sync WebSocket transport](session-sync-transport.md) — replaces bind-mounted session DBs (recurring macOS SQLite corruption / `DB_RETRY_EXHAUSTED`); three canary rounds on 2026-08-16 found and fixed 7 real bugs total, most recently an `INSERT OR IGNORE` silently dropping the staged→pending status flip on already-warm containers (§8.6.3); Lumen is running live on `'sync'` transport with confirmed working round trips across multiple message patterns — next step is running it longer under real traffic before calling it broadly safe (Phase 3+ resync/disconnected-window durability is still unbuilt)
